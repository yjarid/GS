...
/**
* CMB2 code to register CMB2 POST New form field to be used in the front end 
* @param 
* @return 
*/

function cmb2_frontend_form() {
	...
	$cmb->add_field( array(
		'default_cb' => array( $this , 'yj_maybe_set_default_from_posted_values'),
...
function form_shortcode( $atts = array() ) {
	...
	// Get our form
	$output .= cmb2_get_metabox_form( $cmb, 'newRecipe-id', array( 
...
function handle_form_submission() {
	...
	$cmb = $this->yj_recipe_cmb2_get();
		...
		if($_POST['object_id']=='newRecipe-id') {
			$post_data = array();
			...
			$img_id = $this->form_photo_upload( $new_submission_id, $post_data );
...
function yj_recipe_cmb2_get() {
	// Use ID of metabox in cmb2_frontend_form
	$metabox_id = 'front-end-post-form';
	// Post/object ID is not applicable since we're using this form for submission
	$object_id  = 'newRecipe-id';
	// Get CMB2 metabox object
 ...
 */
function form_photo_upload( $post_id, $attachment_post_data = array() ) {
